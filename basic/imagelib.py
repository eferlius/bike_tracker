...
def imagesDictInSubpplots(imagesDict, sharex = True, sharey = True,
    ...
    fig, ax = plots.pltsImg(listImagesDictValues, listTitles = listImagesDictKeys, 
...
def findStartStopValues(array, discValue = 0, maxIntervals = 100):
    ...
    - start contains all the indexes where the array passes from a value 
    below discardedValue to one above
    - stop contains all the indexes where the array passes from a value above
    discardedValue one below
...
def getTLBRprojectionInside(img, discValue = 0, showPlot = False, maxIntH = 1, maxIntV = 3):
    ...
    maxIntH : int, optional
        max intervals of the horizontal projection (on the vertical axis),
        by default 1
    maxIntV : int, optional
        max intervals on the vertical projection (on the horizontal axis), 
        by default 3
...
def sumTLBR(generalTLBR, localTLBR):
    tl_gen = generalTLBR[0]
    br_gen = generalTLBR[1]
    tl_loc = localTLBR[0]
    br_loc = localTLBR[1]
    tl_gen = np.array(tl_gen)
    br_gen = np.array(br_gen)
    tl_loc = np.array(tl_loc)
    br_loc = np.array(br_loc)

    
    tl_tot = (tl_gen+tl_loc).tolist()
    br_tot = (tl_gen+br_loc).tolist()
    
    return tl_tot, br_tot
